...
def index(request):
    ...
    for ids in username_profile:
        ...
        username_pofile_list.append(profile_lists)

    suggestion_final_list= list(chain(*username_pofile_list))



    return render(request, 'index.html', {'user_profile': user_profile, 'posts': feed_list,'suggestion_final_list': suggestion_final_list[:4] })
...
def signin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.info(request, 'you are ANTI-SOCIAL')
